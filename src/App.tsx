...
import VideoRewardsHistory from './pages/VideoRewardsHistory';
...
import Withdrawal from './pages/Withdrawal';import Promo from './pages/Promo';import LoadingSpinner from './components/LoadingSpinner';
  ...
  return (
    <Routes>
      <Route path="/" element={<Layout />}>
        ...
        <Route path="video-rewards-history" element={user ? <VideoRewardsHistory /> : <Navigate to="/login" />} />
        ...
        <Route path="transaction-history" element={user ? <TransactionHistory /> : <Navigate to="/login" />} />
        <Route path="withdraw" element={user ? <Withdrawal /> : <Navigate to="/login" />} />
        ...
        <Route path="admin/users" element={user?.role === 'admin' || user?.role === 'super_admin' || user?.role === 'transaction_admin' ? <AdminUsers /> : <Navigate to="/dashboard" />} />
        <Route path="admin/transactions" element={user?.role === 'admin' || user?.role === 'super_admin' || user?.role === 'transaction_admin' ? <AdminTransactions /> : <Navigate to="/dashboard" />} />
        ...
        <Route path="admin/notifications" element={user?.role === 'admin' || user?.role === 'super_admin' || user?.role === 'transaction_admin' ? <AdminNotifications /> : <Navigate to="/dashboard" />} />
      </Route>
    </Routes>
  );
}
